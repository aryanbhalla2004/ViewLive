...
import ImageUploaderWithCrop from './components/logo-upload/LogoUpload';
...
const LCreate = () => {
  ...
  const [selectedItems, setSelectedItems] = useState<AmenitieInput[]>([]);
  const [business]:any = useOutletContext();
  const {pushDataUser, uploadFile, fetchData} = useContext(DatabaseContext);
  ...
  const [images, setImages] = useState<locationImage>({
    logo: {id: "logo", name: "Logo", width: 250, height: 250, file: ""},
  ...
  const uploadLocation = async (formData: any) => {
    try {
      ...
      const locationResponse = await pushDataUser(createLocation, locations);
  ...
  return (
    ...
    <div className='bbView-container'>
      ...
      <form className='location-form' onSubmit={onSubmit}>
        <div className='single-col-field' ref={sectionRef.profile}>
          ...
          <div className='user-input-bCreate'>
            <div className={error.for === "logoFile" ? 'invalid-input location-logo-upload' : 'location-logo-upload' }>
              <span className='required-patch-absolute'>Required</span>
        ...
        <div className='single-col-field' ref={sectionRef.name}>
          ...
          <div className='user-input-bCreate'>
            <TextInput wordCount={true} wordsAllowed={60} name="locationName" type="text" placeholder='My Restaurent @ Pembina' label='' error={error.for} note={error.message}/>
        ...
        <div className='single-col-field' ref={sectionRef.description}>
          ...
          <div className='user-input-bCreate'>
            <TextAreaInput wordCount={true} wordsAllowed={1500} name="locationDescription" type="text" placeholder='Tell us about ur business?' label='' error={error.for} note={error.message}/>
          </div> 
        </div>
        ...
        <div className='single-col-field' ref={sectionRef.email}>
          ...
          <div className='user-input-bCreate'>
            <TextInput name="locationEmail" type="email" placeholder='mail@example.com' label='' error={error.for} note={error.message}/>
          </div>
        </div>
        <div className='single-col-field' ref={sectionRef.phone}>
          ...
          <div className='user-input-bCreate'>
            <PhoneInputCustom error={error.for} note={error.message} value={phone != undefined && phone?.locationPhone} changeValue={changeValuePhone} type="text" label="" placeholder='+1 (XXX)-XXX-XXXX' name="locationPhone" required={false}/>
          </div>
        </div>
        <div className='single-col-field' ref={sectionRef.website}>
          ...
          <div className='user-input-bCreate'>
            <TextInput name="websiteLink" type="text" placeholder='https://www.exmaple.com/' label='' error={error.for} note={error.message}/>
          </div>
        </div>
        <div className='single-col-field' ref={sectionRef.social}>
          ...
          <div className='user-input-bCreate'>
            <div className='social-box-location-create'>
              ...
              <TextInput name="facebookSocial" type="text" placeholder='mail@example.com' label='' error={error.for} note={error.message}/>
            </div>
            <div className='social-box-location-create'>
              <i className="bi bi-instagram"></i>
              <TextInput name="instagramSocial" type="text" placeholder='mail@example.com' label='' error={error.for} note={error.message}/>
            </div>
            <div className='social-box-location-create'>
              <i className="bi bi-twitter"></i>
              <TextInput name="twitterSocial" type="text" placeholder='mail@example.com' label='' error={error.for} note={error.message}/>
            </div>
            <div className='social-box-location-create'>
              <i className="bi bi-tiktok"></i>
              <TextInput name="tiktokSocial" type="text" placeholder='mail@example.com' label='' error={error.for} note={error.message}/>
            </div>
          </div>
        </div>
          ...
          <div className='user-input-bCreate'>
            <div className='col-2-for-input'>
              <AddressAutofill accessToken="pk.eyJ1IjoiYXJ5YW5iaGFsbGEiLCJhIjoiY2ttbWMxYjN0MG4zNzJ2b2RzenNtNHloeCJ9.D28HxdUCUpf7YpvsQZ26AQ">
                <TextInput autoComplete="address-line1" name="streetName" type="text" placeholder='100 Smith Street' label='' error={error.for} note={error.message}/>
              </AddressAutofill>
              <TextInput autoComplete="postal-code" name="postalcode" type="text" placeholder='Zip Code' label='' error={error.for} note={error.message}/>
            </div>
            
            <div className='col-3-for-input'>
              <TextInput autoComplete="address-level2" name="city" type="text" placeholder='City' label='' error={error.for} note={error.message}/>
              <TextInput autoComplete="address-level1" name="state" type="text" placeholder='State/Province' label='' error={error.for} note={error.message}/>
              <TextInput autoComplete="country" name="country" type="text" placeholder='Country' label='' error={error.for} note={error.message}/>
            </div>
          </div>
        </div>
        <div className='single-col-field' ref={sectionRef.live}>
          ...
          <div className='user-input-bCreate'>
            <TextInput name="livelink" type="text" placeholder='https://www.exmaple.com/' label='' error={error.for} note={error.message}/>
          </div>
        </div>
              ...
              <li key={index} className={error.for === item[0] ? "invalid-input-time" : ""}>
                ...
                <div className="checkboxes__item">
                  <label className="checkbox style-e">
                    <input type="checkbox" checked={times[item[0]] != null} name={item[0]} onChange={changeOpens}/>
                ...
                {times[item[0]] != null ? <>
                    <input type="time" className='form-control' name="start" onChange={e => changeInterval(e, item[0])}></input>
                    <span>TO</span>
                    <input type="time" className='form-control' name="end" onChange={e => changeInterval(e, item[0])}></input>
          ...
          <div className='label-field-holder'>
            <div className="checkboxes__item">
              <label className="checkbox style-e">
                ...
                <div className="checkbox__checkmark"></div>
                <div className={error.for === "terms" ? 'term-error checkbox__body' : 'checkbox__body'}>I Agree with Terms & Conditions</div>
              </label>
            </div>
          </div>
          <div className='user-input-bCreate'>
            <Button text='Create Location' loading={formSub}/>
          </div>
        </div>
    ...
    {showLoading && <div className='create-location-message-pop-up'>
      ...
      <div className={loadingPost ? "content-container-loader-single-page hidevisiility" : "content-container-loader-single-page"}>
          ...
          <p className='reference-l'>location ref: 2132312-4534512-34242</p>
          <h2>You successfully created your location</h2>
          <p className='des-location-pop'>Congratulations on creating a new location! You can now easily manage and close it by clicking the button. Well done!</p>
