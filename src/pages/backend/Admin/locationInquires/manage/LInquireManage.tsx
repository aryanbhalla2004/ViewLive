...
import "./style.css";
import {getBusiness, getLocation,  getUser as getUserDB} from "../../../../../graphql/queries"
...
import {updateBusiness, updateLocation} from "../../../../../graphql/mutations";
import { Business, Location, Status, UpdateBusinessInput, UpdateLocationInput, User } from '../../../../../API';
...
const LInquireManage = () => {
  const navigate = useNavigate();
  const {pullObj, pushDataUser, fetchProtectedObject, updateLocationFunction} = useContext(DatabaseContext);
  ...
  const pull = async () => {
    const response = await pullObj(getLocation, id);
    const location : Location = response?.data.getLocation;
    if(location.status === Status.WAITING) {
      setData(location);
    } else {
      navigate("/dashboard/admin/location-inquires");
    }
  }
  ...
  const changeStatus = async (status:Status, reason?: string) => {
    if(data != undefined) {
      ...
      const locationDetails: UpdateLocationInput = {
        id: data.id,
        status: status,
      };
      
      try {
        const updateDataBase = await updateLocationFunction(updateLocation, locationDetails);
        const response = await pullObj(getUserDB, data.owner);
        const locationOwner:User = response.data.getUser; 
        const emailData = await getEmailData(locationOwner, data, status, reason);
    ...
    <div className='bbView-container'>
      <div className='dash-header-page'>
        <div className='name-path-dash-page'>
          <h2>{data?.name}</h2>
          <p className='path-style'>Dashboard <i className="bi bi-chevron-right"></i> Location Inquires <i className="bi bi-chevron-right"></i> Manage</p>
        </div>
        <div className='action-button-container-dash'>
          <Link to="/dashboard/admin/location-inquires" className='link-button btn-primary'><i className="bi bi-arrow-left"></i> Cancel</Link>
          <button onClick={() => showDecisionBox("APPROVE")} className='link-button approve-button'>Approve Location</button>
          <button onClick={() => showDecisionBox("DECLINE")} className='link-button reject-button'>Reject Location</button>
        </div>
      </div>
...
export default LInquireManage;


const getEmailData = (user: User, data: Location, accept: Status, reason?: string) : String => {
